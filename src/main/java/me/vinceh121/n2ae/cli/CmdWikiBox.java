...
public class CmdWikiBox implements Callable<Integer> {
	...
	public Integer call() throws Exception {
		...
		final Map<String, NOBClazz> model =
				this.mapper.readValue(this.modelInput, new TypeReference<Map<String, NOBClazz>>() {
				});

		for (final File input : this.inputs) {
			final IParser parser;
			if (input.getName().endsWith(".n")) {
				parser = new NOBParser();
			} else if (input.getName().endsWith(".tcl")) {
